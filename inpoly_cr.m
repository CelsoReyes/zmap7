function [in_new,bnd_new] = inpoly_cr(p, node, cnect)
% UNDER CONSTRUCTION AND JUST GENERALLY BEING TESTED
...
disp('new:');
tic
% up to now, all Cb,Db,etc. are Nx1, therefore,
in_effective_lower_half = p(:,2)<=mnyp;
xb = p(in_effective_lower_half,1)'; 
yb = p(in_effective_lower_half,2)';
ontestb = abs((Fb-yb) .* (Cb-xb) - (Db-yb) .* (Eb-xb)) < tol; % walls x points

ub = ((Eb-Cb).*(Db-yb)-(Fb-Db).*(Cb-xb))./(-(X-xb).*(Fb-Db));
cnb = sum(ub > -tol & ub < (1+tol) );
%whos
bndb = any(ontestb); % t/f vertical matrix, for each point.
insideb = mod(sum(cnb),2) | bndb;
bnd_new(in_effective_lower_half)=bndb;
in_new(in_effective_lower_half)=insideb;
%clear xb yb ontestb bndb insideb

xt = p(~in_effective_lower_half,1)';
yt = p(~in_effective_lower_half,2)';
ontestt = abs((Ft-yt) .* (Ct-xt) - (Dt-yt) .* (Et-xt)) < tol;
ub = ((Eb-Cb).*(Db-yt)-(Fb-Db).*(Cb-xt))./(-(X-xt).*(Fb-Db));
cnt = sum(ub > -tol & ub < (1+tol) );
bndt = any(ontestt);
insidet = mod(sum(cnt),2) | bndt;
bnd_new(~in_effective_lower_half)=bndt;
in_new(~in_effective_lower_half)=insidet;
%clear xt yt ontestt bndt insidet
bnd_new = bnd_new';
in_new = in_new';
toc
...
toc
whos
if ~isequal(bnd,bnd_new)
    disp('different answers for boundaries!')
else
    disp('boundaries match')
end
if ~isequal(in, in_new)
    disp('different answers for inside!');
else
    disp('inside matches')
end
return
