function handles=focalmech(fm, centerX, centerY, diam, varargin)
    ...
    %   9/19/2017 Added ability to return graphic handles. -Celso Reyes
    ...
    handles=struct('circle',[],'fill',[],'text',[]);
    ...
    if eig1 <= 0     % implosion
        handles.fill=fill(ax,centerX+u,centerY+w,'w')   % fill white circle
        return
    end
    if eig3 >= 0     % explosion
        handles.fill=fill(ax,centerX+u,centerY+w,fillcolor)   % filled circle
        return
    end
    ...
    if ~colorit
        ...
        i = c(2,:) >= 1;
        c(:,i) = nan;
        handles.circle(2)=plot(ax,centerX+sE*c(1,:),centerY+sN*c(2,:),'color',fillcolor);  % plot circle boundary
    else
        handles.fill=fill(ax,centerX+u,centerY+w,'w');  % fill white circle
        handles.circle=plot(ax,centerX+u,centerY+w,'k');   % plot circle boundary
        ...
        if nc == 1       % 1 contour (closed unless single vertical plane)
            ...
            if d2 > 2 % single vertical plane
                ...
                ac = [cos(a1) sin(a1)];
                c1 = [c; ac];
                handles.fill(end+1)=fill(ax,centerX+sE*c1(:,1),centerY+sN*c1(:,2),fillcolor);
                ...
                if uv > 0   % fill inside contour
                    handles.fill(end+1)=fill(ax,centerX+sE*c(:,1),centerY+sN*c(:,2),fillcolor);
                elseif uv < 0   % fill outside contour
                    handles.fill(end+1)=fill(ax,centerX+u,centerY+w,fillcolor);
                    handles.fill(end+1)=fill(ax,centerX+sE*c(:,1),centerY+sN*c(:,2),'w');
                end
            end
        else                    % 2 contours
            ...
            c1 = [c(2:ia(2),:); ac; c(2,:)];
            if uvc < 0
                handles.fill(end+1)=fill(centerX+u,centerY+w,fillcolor);   % fill colored circle
                handles.fill(end+1)=fill(centerX+sE*c1(:,1),centerY+sN*c1(:,2),[ 1 1 1]);
            else
                handles.fill(end+1)=fill(centerX+sE*c1(:,1),centerY+sN*c1(:,2),fillcolor);
            end
            ...
            if uvc < 0
                handles.fill(end+1)=fill(centerX+sE*c2(:,1),centerY+sN*c2(:,2),'w');
            else
                handles.fill(end+1)=fill(centerX+sE*c2(:,1),centerY+sN*c2(:,2),fillcolor);
            end
    ...
    if ~isempty(ctitle)
        handles.text=text(centerX,centerY+sN*1.1,ctitle,'HorizontalAlignment','center','FontSize',fsize);
    end
