    ...
    switch version
        ...
        case 'bcross'
            savestuff =[ 'ZmapMessageCenter.set_info(''Save Grid'',''  '');',...
                ...
                'sapa2=[''save '' path1 file1 '' ll a newgri lat1 lon1 lat2 lon2 ZG.xsec_defaults.WidthKm  bvg xvect yvect gx gy dx dd ZG.bin_dur newa maex maey maix maiy ''];',...
                ' if length(file1) > 1, eval(sapa2),end , '];
            
        case 'bcrossV2'
            savestuff = [ 'ZmapMessageCenter.set_info(''Save Grid'',''  '');',...
                '[file1,path1] = uiputfile(fullfile(ZmapGlobal.Data.Directories.data, ''*.mat''), ''Grid Datafile Name?'') ;',...
                ' sapa2 = [''save '' path1 file1 '' ll a tmpgri newgri lat1 lon1 lat2 lon2 ZG.xsec_defaults.WidthKm  bvg xvect yvect gx gy dx dd ZG.bin_dur newa maex maey maix maiy ''];',...
                ' if length(file1) > 1, eval(sapa2),end , '];
        ...
        case 'bvalgrid'
            savestuff=[ 'ZmapMessageCenter.set_info(''Save Grid'',''  '');',...
                '[file1,path1] = uiputfile(fullfile(ZmapGlobal.Data.Directories.data, ''*.mat''), ''Grid Datafile Name?'') ;wholePath=[path1 file1]; ',...
        ...
        case 'calc_across'
            savestuff=[ 'ZmapMessageCenter.set_info(''Save Grid'',''  '');',...
                '[file1,path1] = uiputfile([ ''*.mat''], ''Grid Datafile Name?'') ;',...
                'sapa2=[''save '' path1 file1 '' ll a tmpgri newgri lat1 lon1 lat2 lon2 ZG.xsec_defaults.WidthKm  avg xvect yvect gx gy dx dd ZG.bin_dur newa maex maey maix maiy ''];',...
                ' if length(file1) > 1, eval(sapa2),end , '];
            
        case 'calc_Omoricross'
            savestuff=[ 'ZmapMessageCenter.set_info(''Save Grid'',''  '');',...
                '[file1,path1] = uiputfile(fullfile(hodi, ''eq_data'', ''*.mat''), ''Grid Datafile Name?'') ;',...
                ' sapa2 = [''save '' path1 file1 '' mCross gx gy dx dy ZG.bin_dur tdiff t0b teb newa a main faults mainfault coastline yvect xvect tmpgri ll ZG.overall_b_value newgri ra time timef bootloops ZG.maepi xsecx xsecy ZG.xsec_defaults.WidthKm lon1 lat1 lon2 lat2 ''];',...
                ' if length(file1) > 1, eval(sapa2),end , '];
