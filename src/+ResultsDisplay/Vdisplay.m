classdef Vdisplay < ResultsDisplay.ZmapResultsPlugin
    ...
    properties(SetObservable, AbortSet)
        ...
        SelectionColors = [FancyColors.rgb('blue');
        FancyColors.rgb('mandy')];
    end
    ...
    2.  a field called Grid, which is a ZmapGrid and contains fields X, Y
        ...
        function updateClickPoint(obj,~,ev)
            ...
            if withinAxes
                ...
                mx = pt(1);
                my = pt(2);
                [~,nearest]=min((mx-obj.Data.distance_along_strike).^2 + (my-obj.Data.z).^2);
                x = obj.Data.distance_along_strike(nearest);
                y = obj.Data.z(nearest);
        ...
        function mycolor = get.colorForThisPoint(obj)
            ...
            switch obj.ColorBy
                case 'result'
                    ...
                    gridx = find(gr.XData==obj.Parent.samplePoints(obj.Parent.pointChoice).X &...
                        gr.YData==obj.Parent.samplePoints(obj.Parent.pointChoice).Y);
    ...
    methods
        ...
        function updateRing(obj, tb, sp)
            ...
            if obj.showRing && ~isempty(tb)
                % update samplecircle
                refEl = obj.Parent.RawCatalog.RefEllipsoid;
                sampPts = (0:2:360)';
                r = obj.Data.RadiusKm(obj.Parent.nearestSample);
                assert(refEl.LengthUnit == "kilometer");
                xys = [cosd(sampPts), sind(sampPts)].* r + [tb.distance_along_strike, tb.z];
                set(CR, 'XData', xys(:,1), 'YData', xys(:,2), 'LineStyle', '--');
            else
        ...
        function updateSeries(obj, sp)
            ...
            set(findobj(obj.Parent.ax, 'Tag',['selection', obj.Parent.pointChoice]),'XData', c.DistAlongStrike', 'YData', c.Z);
