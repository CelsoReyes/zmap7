function timemagplot(obj, tabgrouptag)
    % Cumulative Event Plot
    
    Tags.xs = 'TMPlot xs contextmenu';
    Tags.bg = 'TMPlot bg contextmenu';
    Tags.line = 'TMPlot line contextmenu';
    Tags.ax = 'TMPlot axes';
    
    % set up the axes
    myTab = findOrCreateTab(obj.fig, obj.fig, tabgrouptag, 'Time-Mag');
    ...
    if isempty(cxs)
        ...
        uimenu(cxs,'Label','Open in new window',MenuSelectedField(),@cb_xstimedepthplot);
    end
    
    obj.plot_xsections(@xsplotter, 'Xsection timemagplot');
    ...
    if isempty(cbg)
        ...
        uimenu(cbg, 'Label', 'Open in new window', MenuSelectedField(),@obj.cb_timedepthplot);
    end
    ...
    function cln = line_plot_context_menu()
        ...
        if isempty(cln)
            ...
            uimenu(cln, 'Label', 'Open in new window',      MenuSelectedField(), @obj.cb_timedepthplot);
        end
    end
