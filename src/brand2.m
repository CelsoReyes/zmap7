function brand2() % autogenerated function wrapper
    ...
    lengtha = ZG.primeCatalog.Count;
    conca = ZG.primeCatalog;
    ...
    %TOFIX assigning to a section of ZmapCatalog, or concatinating. not implemented
    error('Catalogs cannot yet be concatinated as such')
    %paste the two catalogs to get a concatenated set
    ZG.primeCatalog(lengtha+1:lengtha*2,:) = conca;
    
    
    minnu = 40;  % minimum number of events in each period
    t4 = ZG.primeCatalog.Date(end)
    t3 = ZG.primeCatalog.Date(lengtha+1);
    t2 = ZG.primeCatalog.Date(lengtha);
    t1 = ZG.primeCatalog.Date(1);
