function newsta(sta) % autogenerated function wrapper
    ...
    t0b = min(ZG.newt2.Date); % changed from ZG.primeCatalog.Date
    teb = max(ZG.newt2.Date); % changed from ZG.primeCatalog.Date
    ...
    switch sta
        ...
        case 'lta'
            ...
            probabilityButtonCallback=@(~,~)translating(ZG.newcat, as,'z');
            
        case 'bet'
            as=betfun(winlen_days, cumu, ZG.newcat, length(xt));
            titletext=['LTA(t) Function; \beta-values; wl = ', char(ZG.compare_window_dur)];
            probabilityButtonCallback=@(~,~)translating(ZG.newcat, as,'beta');
    end
