...
if newbmapWindowFlag
    ...
    uimenu(options,'Label','Select EQ in Circle',...
         'Callback','h1 = gca;ho=''noho'';cirbva;watchoff(bmap)')
    ...
    uimenu(op1,'Label','max magnitude map',...
         'Callback',' lab1=''Mmax'';re3 = maxm; view_bva')
    uimenu(op1,'Label','magnitude range map (Mmax - Mcomp)',...
         'Callback',' lab1=''dM '';re3 = maxm-magco; view_bva')

    uimenu(op1,'Label','recurrence time map ',...
         'Callback','def = {''6''};m = inputdlg(''Magnitude of projected mainshock?'',''Input'',1,def);m1 = m{:}; m = str2num(m1);lab1 = ''Tr (yrs) (sm. values only)'';re3 =(teb - t0b)./(10.^(avm-m*old)); view_bva')
...
set(ploeq,'Tag','plot_eq','MarkerSize',ms6,'Marker',ty,'Color',co,'Visible',vi)
