function zgrid3d(sel) % autogenerated function wrapper
    ...
    if sel == 'ca'
        ...
        for il =1:length(t5)
            ...
            di = sqrt(((ZG.primeCatalog.Longitude-x)*cosd(y)*111).^2 + ((ZG.primeCatalog.Latitude-y)*111).^2 + ((ZG.primeCatalog.Depth - z)).^2 ) ;
            [s,is] = sort(di);
        ...
        try
            ...
            if length(file1) > 1
                save([path1 file1], 'zvg', 'ram', 'gx', 'gy', 'gz', 'dx', 'dy', 'dz',...
                    ...
                    'coastline', 'yvect', 'xvect', 'tmpgri', 'll'); %TOFIX savevariables
