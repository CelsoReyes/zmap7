% TODO: convert this to a ZmapHGridFunction
function bvg = stressgrid % autogenerated function wrapper
    ...
    function my_calculate()
        ...
        obj.Result.values.x = Grid.X(:);
        obj.Result.values.y = Grid.Y(:);
        ...
        view_stressmap(bvg, obj.Result.values)
...
function view_stressmap(bvg, values) % autogenerated function wrapper
    % view_stressmap
    ...
    %normlap2(ll)= values.Radius_km;%bvg(:,8);
    %rama=reshape(normlap2,length(yvect),length(xvect));
    
    %normlap2(ll)= values.Variance; %bvg(:,7);
    %r=reshape(normlap2,length(yvect),length(xvect));
    
    %normlap2(ll)= values.S1Trend; %bvg(:,1)
    %s11=reshape(normlap2,length(yvect),length(xvect));
    
    %normlap2(ll)= values.S2Plunge; %bvg(:,4)
    %s31=reshape(normlap2,length(yvect),length(xvect));
    
    %normlap2(ll)= values.S1Plunge; %bvg(:,2);
    ...
    plq = quiver(values.x(l_notnormal),values.y(l_notnormal),-cos(sor(l_notnormal,SA*2)*pi/180),sin(sor(l_notnormal,SA*2)*pi/180),0.6,'.');
    ...
    plq_n = quiver(values.x(l_normal),values.y(l_normal),-cos(sor(l_normal,SA2*2)*pi/180),sin(sor(l_normal,SA2*2)*pi/180),0.6,'.');
    ...
    axis([ s2 s1 s4 s3])
    title(sprintf('%s;  %g to %g', name, t0b, teb),'FontSize',ZmapGlobal.Data.fontsz.s,...
