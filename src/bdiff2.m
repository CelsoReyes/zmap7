classdef bdiff2
    ...
    methods
        ...
        function plot(obj,catalog, ax)
            ...
            set(gca,'visible','on','FontSize',fs,'FontWeight','normal',...
            ...
            if isempty(ax.UIContextMenu)
                ...
                uimenu(ax.UIContextMenu,'Separator','on','Label','info','callback',@(~,~)msgbox(tx,'b-Value results','modal'));
        end
        ...
        function create_my_menu(obj,c,catalog)
            uimenu(c,'Label','Estimate recurrence time/probability', 'callback',@callbackfun_recurrence);
            uimenu(c,'Label','Plot time series', 'callback',@callbackfun_ts);
            uimenu(c,'Label','Examine Nonlinearity (optimize  Mc)','Callback',{@cb_nonlin_optimize,catalog});
