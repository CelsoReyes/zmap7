function view_bva(lab1, re3)
    ...
    if newbmapWindowFlag
        ...
        uimenu(options,'Label','Refresh ', 'Callback','view_bva(lab1,re3)')
        ...
        adjmenu =  uimenu(op1,'Label','Adjust Map Display Parameters');
            ...
            'Callback','asel = ''mag''; adju(); view_bva(lab1,re3) ')
        uimenu(adjmenu,'Label','Adjust Rmax cut',...
            'Callback','asel = ''rmax''; adju(); view_bva(lab1,re3)')
        uimenu(adjmenu,'Label','Adjust goodness of fit cut',...
            'Callback','asel = ''gofi''; adju(); view_bva(lab1,re3) ')
        
        
        uimenu(op1,'Label','b-value map (max likelihood)',...
            'Callback','lab1 =''b-value''; re3 = mBvalue; view_bva(lab1,re3)')
        uimenu(op1,'Label','Standard deviation of b-Value (max likelihood) map',...
            'Callback','lab1=''SdtDev b-Value''; re3 = mStdB; view_bva(lab1,re3)')
        uimenu(op1,'Label','Magnitude of completness map',...
            'Callback','lab1 = ''Mcomp''; re3 = mMc; view_bva(lab1,re3)')
        uimenu(op1,'Label','Standard deviation of magnitude of completness',...
            'Callback','lab1 = ''Mcomp''; re3 = mStdMc; view_bva(lab1,re3)')
        uimenu(op1,'Label','Goodness of fit to power law map',...
            'Callback','lab1 = '' % ''; re3 = Prmap; view_bva(lab1,re3)')
        uimenu(op1,'Label','Resolution map',...
            'Callback','lab1=''Radius in [km]'';re3 = r; view_bva(lab1,re3)')
        uimenu(op1,'Label','Earthquake density map',...
            'Callback','lab1=''log(EQ per km^2)'';re3 = log10(mNumEq./(r.^2*pi)); view_bva(lab1,re3)')
        uimenu(op1,'Label','a-value map',...
            'Callback','lab1=''a-value'';re3 = mAvalue; view_bva(lab1,re3)')
        ...
        if exist('mStdDevB')
            ...
            uimenu(AverageStdDevMenu,'Label', 'Bootstrapped standard deviation of b-value',...
                'Callback','lab1=''standard deviation of b-value''; re3 = mStdDevB; view_bva(lab1,re3)')
            uimenu(AverageStdDevMenu,'Label', 'Bootstrapped standard deviation of Mc',...
                'Callback','lab1=''standard deviation of Mc''; re3 = mStdDevMc; view_bva(lab1,re3)')
            uimenu(AverageStdDevMenu,'Label', 'b-value map (max likelihood) with std. deviation',...
                'Callback','lab1=''b-value''; re3 = mBvalue; bOverlayTransparentStdDev = 1; view_bva(lab1,re3)')
        end
        
        recmenu =  uimenu(op1,'Label','recurrence time map '),...
            
    uimenu(recmenu,'Label','recurrence time map ',...
        'Callback','def = {''6''};m = inputdlg(''Magnitude of projected mainshock?'',''Input'',1,def);m1 = m{:}; m = str2num(m1);lab1 = ''Tr (yrs) (sm. values only)'';re3 =(teb - t0b)./(10.^(mAvalue-m*mBvalue)); mrt = m; view_bva(lab1,re3)')
    
    uimenu(recmenu,'Label','(1/Tr)/area map ',...
        'Callback','def = {''6''};m = inputdlg(''Magnitude of projected mainshock?'',''Input'',1,def);m1 = m{:}; m = str2num(m1);lab1 = ''1/Tr/area '';re3 =(teb - t0b)./(10.^(mAvalue-m*mBvalue)); re3 = 1./re3/(2*pi*ra*ra);  mrt = m; view_bva(lab1,re3)')
