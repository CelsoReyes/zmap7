function plot_bootfitloglike_a2b(a,time,timef,bootloops,maepi)
    % function plot_bootfitloglike_a2b(a,time,timef,bootloops,maepi);
    % --------------------------------------------------
...
report_this_filefun(mfilename('fullpath'));
    ...
    if (isnan(pval1) == 0 & isnan(pval2) == 0)
        ...
        loopout = [loopout , loopout(:,1)*0, loopout(:,1)*0];
        mModels = [];
        ...
        for j = 1:length(loopout(:,1))
            ...
            hold on
            mModels = [mModels cumnr_model];
        end
        for i=1:length(mModels(:,1))
            vDist=mModels(i,:);
            vStd(i) = calc_StdDev(vDist);
        end
        ...
        p1 = plot(time_as,cumnr_model,'Linewidth',2,'Linestyle','--','Color',[0 0.6 0]);
        hold on;
        p2 = plot(time_as,cumnr,'Linewidth',2,'Linestyle','--','Color',[0 0 0]);
        ...
        pmedmod =  plot(time_asf,cumnr_modelmed,'Linestyle','-.','Linewidth',2,'Color',[1 0 0]);
        %     pfStd1 = plot(time_asf,cumnr_modelmed-vStd','Linestyle','--','Linewidth',2,'Color',[0.4 0.4 0.4]);
        %     pfStd2 = plot(time_asf,cumnr_modelmed+vStd','Linestyle','--','Linewidth',2,'Color',[0.4 0.4 0.4]);
        ...
        pf3 = plot(vTime_forecast, vCumnr_forecast,'Linestyle','-.','Linewidth',2,'Color',[0 0 0]);
        ...
        set(ps2,'Linewidth',4,'Color',[1 0 0])
        %
        %     % Title
        ...
        % Legend
        %legend([p2 pf3 p1 pmedmod min(ps2) pfStd1 paf],'Data tL','Data tF','Model to data', 'Mean Bst-model', '\sigma (Bst)', '\sigma (Bst)','Sec. AF',0);
        legend([p2 pf3 p1 pmedmod min(ps2) paf],'Data tL','Data tF','Model to data', 'Mean Bst-model', '\sigma (Bst)','Sec. AF', 'location', 'Best');
