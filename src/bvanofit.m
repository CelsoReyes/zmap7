function bvanofit(mycat) % autogenerated function wrapper
    ...
    if isempty(mycat), mycat = a; end
    [maxmag, minmag2] = mycat.MagnitudeRange();
    n = mycat.Count;
    ...
    l = mycat.Date > t1p(1) & mycat.Date < t2p(1) ;
    ...
    [cumux, xt] = hist(mycat.Date(l),t1p(1):days(ZG.bin_dur):t2p(1));
    
    l = mycat.Date > t3p(1) & mycat.Date < t4p(1) ;
    ...
    [cumux2, xt] = hist(mycat.Date(l),t3p(1):days(ZG.bin_dur):t4p(1));
    mean1 = mean(cumux);
    mean2 = mean(cumux2);
    var1 = cov(cumux);
    var2 = cov(cumux2);
