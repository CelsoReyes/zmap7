...
if sel == 'i1'
    ...
    go_button1=uicontrol('Style','Pushbutton',...
        ...
        'Units','normalized',...
        'Callback','inb1 =get(hndl2,''Value'');tgl1 =get(tgl1,''Value'');tgl2 =get(tgl2,''Value'');close,sel =''in'', reso3d',...
        'String','Go');
...
if sel == 'ca'
    ...
    %
    bvg = NaN(length(gx),length(gy),length(gz));
    bvg_wls = NaN(length(gx),length(gy),length(gz));

    ram  = NaN(length(gx),length(gy),length(gz));
    go  = NaN(length(gx),length(gy),length(gz));
