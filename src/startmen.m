function startmen(parent_fig)
    ...
    if nargin==1
        ...
        uimenu(genmen, ...
            ...
            MenuSelectedFcnName(), @(~,~)cb_load_file);%ZmapImportManager(@load_zmapfile));
        uimenu(genmen, ...
        ...
        uimenu(genmen, ...
            ...
            MenuSelectedFcnName(), @(~,~)cb_load_web);%ZmapImportManager(@get_fdsn_data_from_web_callback));
        uimenu(genmen, ...
            'Label', 'Create or Modify *.mat datafile',...
            MenuSelectedFcnName(), {@think_and_do, 'setup'});
        uimenu(genmen, ...
            'Label', 'Current Dataset Info',...
            'Enable','off',... % may no longer be relevent
            MenuSelectedFcnName(), @(~,~)datinf());
        
        uimenu(genmen, ...
            'Label','Set working directory ',...
            'Enable','off',... % doesn't seem to be used anywhere
            MenuSelectedFcnName(),@(~,~)working_dir_in);
        ...
        uimenu(genmen, ...
            ...
            MenuSelectedFcnName(), @(s,e) showweb('new'));
        
        uimenu(genmen, ...
            'Label','Sample Slide Show',...
            MenuSelectedFcnName(),@(s,e) slshow());
