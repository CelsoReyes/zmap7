function [mCatclus] =  clusterGK(mCatalog, Mainmag, Mc, t1, t2)

% Example [mCatclus] =  clusterGK(b, 1.55, 1.55, 1, 1);
...
%     end
    if (b(i,12) == 0 && b(i,6) > Mainmag)
        ...
        latref = b(i,2);
        lonref = b(i,1);
        searchradius=10^(0.1238*magref+0.983); %search radius according to Uhrhammer
        ...
        while b(lino,3) < (tref+Dtafter) && (lino+1 < le)
            if b(lino,12) ==0
                if b(lino,6) > magref
                    searchradius=10^(0.1238*magref+0.983);
                end
