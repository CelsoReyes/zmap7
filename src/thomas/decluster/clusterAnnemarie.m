function [mCatclus] =  clusterGeneral(dMethod, mCatalog, Mainmag, Mc, t1, t2)
    ...
    switch dMethod
        case DeclusterTypes.Utsu
            searchradiusFcn = @(magref) 10^(0.5*magref-1.5965); %search radius according to Uhrhammer
        case DeclusterTypes.Gardner_Knopoff_clusterGK
            searchradiusFcn = @(magref) 10^(0.1238*magref+0.983); %search radius according to Uhrhammer
        case DeclusterTypes.Uhrhammer
            searchradiusFcn = @(magref) 10^(0.35*magref-0.44);
        otherwise
            error('This only interprets DeclusterTypes:Utsu, GardnerKnopoff_clusterGK,  UhrHammer')
    end
    for i = 1:le
        if (b(i,12) == 0 && b(i,6) > Mainmag)
            ...
            latref = b(i,2);
            lonref = b(i,1);
            searchradius=searchradiusFcn(magref);
