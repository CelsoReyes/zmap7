function [ mSynCat, vMain] = calc_SynCatSRC(nNbkg,vSynCat,nSynCat_,nSynMode,...
    vAfter,vEtas,mCatalog,bPSQ,vPSQ,mPSQ)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
switch nSynCat_
    case 1
        ...
        mSynCat=syn_catalog(nNbkg, fBValue, fMc, fInc, fMinLat,fMaxLat,...
            ...
            mCatalog);
        if bPSQ
            mSynCat=sr_makeSRC(mSynCat,vPSQ(2),vPSQ(3),vPSQ(4),...
                vPSQ(1),mPSQ);
        end
%       mSynCat(:,1)=mSynCat(randperm(size(mSynCat,1))',1);
        vMain=ones(size(mSynCat,1),1);
...
%         mCat(:,1)=mCat(randperm(size(mCat,1))',1);
        ...
        mCat=mCat(Idx,:);
        if bPSQ
            mCat=sr_makeSRC(mCat,vPSQ(2),vPSQ(3),vPSQ(4),...
                vPSQ(1),mPSQ);
        end
...
%         mCat1(:,1)=mCat1(:,1)-(max(mCat1(:,1))-min(mCat1(:,1)));
        ...
        fprintf('The synthetic catalog consists of %5.0f earthquakes\n', size(mSynCat,1));
        vMain=(catalog(:,12)==0);
end
% disp('cat fertig')
