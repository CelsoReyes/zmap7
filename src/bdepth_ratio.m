function bdepth_ratio() % autogenerated function wrapper
    ...
    error('this needs to be updated to the new catalog version')
    ...
    % to each grid point
        ...
        %
        % overall b-value
        [bv magco stan av pr] =  bvalca3(top_zone.Magnitude,ZG.inb1);
        tbo1 = bv; tno1 = length(top_zone(:,1));
        
        [bv magco stan av pr] =  bvalca3(bot_zone.Magnitude,ZG.inb1);
        ...
        for i= 1:length(newgri(:,1))
            ...
            if length(topb) >= Nmin  && length(botb) >= Nmin
                ...
                elseif ZG.inb1 == 1
                    % ZG.newt2 = topb;
                    % [Mc, Mc90, Mc95, magco, prf]=mcperc_ca3(topb.Magnitude);
                    [topbv magco topstan topav topme topmer topme2,  toppr] =  bvalca3(topb.Magnitude,1);
                    l = topb(:,6) >= magco-0.05;
                    if length(topb(l,:)) >= Nmin
                        [topbv2 topstan2 topav2] =  bvalca3(topb(l,:));
                        n1 = length(topb(l,:));
                    ...
                    %    ZG.newt2 = botb;
                    %    [Mc, Mc90, Mc95, magco, prf]=mcperc_ca3(botb.Magnitude);
                    [botbv magco botstan botav botme botmer botme2,  botpr] =  bvalca3(botb.Magnitude,1);
                    l = botb(:,6) >= magco-0.05;
                    if length(botb(l,:)) >= Nmin
                        [botbv2 botstan2 botav2] =  bvalca3(botb(l,:));
                        n2 = length(botb(l,:));
                ...
                elseif ZG.inb1 == 2
                    [topbv magco topstan topav topme topmer topme2,  toppr] =  bvalca3(topb.Magnitude,2);
                    n1 = length(topb);
                    [topbv2 topstan2 topav2] = calc_bmemag(topb.Magnitude);
                    
                    [botbv magco botstan botav botme botmer botme2,  botpr] =  bvalca3(botb.Magnitude,2);
                    
                    n2 = length(botb);
                    [botbv2 botstan2 botav2] = calc_bmemag(botb.Magnitude);
