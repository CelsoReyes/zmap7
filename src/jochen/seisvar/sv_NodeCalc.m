...
% Changes:
% 19.08.02: Replaced  fcumulsum.m with calc_cumulsum.
...
elseif (bSeisModel == 1)
    ...
    % Determine Mc for entire catalog, two time periods and differenc in Mc
...
%     result.dNdiff = max(dEv_val2)/result.fSecondPeriodExact-max(dEv_val)/result.fFirstPeriodExact; % Normalization
    ...
    if isempty(result.fMc)
     ...
     result.fMcFirstPeriod = calc_Mc(result.mFirstCatalog, nCalculateMC);
%    result.fMcFirstPeriod = calc_McCdf2(result.mFirstCatalog, 0.1);
    if isempty(result.fMcFirstPeriod)
        result.fMcFirstPeriod = NaN;
    end
     result.fMcSecondPeriod = calc_Mc(result.mSecondCatalog, nCalculateMC);
%    result.fMcFirstPeriod = calc_McCdf2(result.mSecondCatalog, 0.1);
    if isempty(result.fMcSecondPeriod)
        result.fMcSecondPeriod = NaN;
    end
    result.fdMc = result.fMcSecondPeriod - result.fMcFirstPeriod;
    ...
    [fProb_nochange, fBic_nochange] = calc_loglikelihood_nochange(result.mFirstCatalog, result.mSecondCatalog);
    [fdM, fProb_dM, fBic_dM, mLikeli_dM] = calc_loglikelihood_dM(result.mFirstCatalog, result.mSecondCatalog);
    %[fS, fProb_stretch, fBic_stretch, mLikeli_dS] = calc_loglikelihood_stretch(result.mFirstCatalog, result.mSecondCatalog);
    [fFac, fProb_Rate, fBic_Rate, mLikeli_Rate] = calc_loglikelihood_rate(result.mFirstCatalog, result.mSecondCatalog);
    [fdM_rate, fdM_Fac, fProb_dMrate, fBic_dMrate, mLikeli_dMrate] = calc_loglikelihood_dM_rate(result.mFirstCatalog, result.mSecondCatalog);
