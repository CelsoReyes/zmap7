function [fCumMoment, vCumMoment, vMoment] = calc_moment(mCatalog)
    % Calculates the (cumulative) moment release in Nm of the catalog
    % [fCumMoment, vCumMoment, vMoment] = calc_moment(mCatalog);
    % -------------------------------------------------------------------------
    %
    % Calculates the (cumulative) moment release in Nm of the catalog using either a vector of magnitudes or a ZMAP catalog format
    ...
    if isa(mCatalog,'ZmapBaseCatalog') && mCatalog.Count >= 1
