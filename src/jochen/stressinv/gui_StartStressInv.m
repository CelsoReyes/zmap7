function gui_StartStressInv(mCatalog, hFigure, bMap, rContainer,vCoastline,vFaults)
% function gui_StartStressInv(mCatalog, hFigure, bMap, rContainer,vCoastline,vFaults)
% -------------------------------------------------------------------------
% Start stress tensor inversion toolbox
%
% Input parameters:
%   mCatalog      Earthquake catalog to be analyzed
%                 NOTE: Column 10: dip direction (East of North)
%                       Column 11: dip
%                       Column 12: rake
...
if ~ishandle(hMenuFig)
    ...
    if answer == 1
        ...
        if bSaveParameter
            ...
            [mResults] = gui_CalcStressInv(params);
            % Create output
            sv_result(mResults);
        end
    else
