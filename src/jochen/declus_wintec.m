...
fEventpercentage = 100*length(mCatAfter(:,1))/mCatalog.Count; % Percentage of events in clusters
...
sInfost1 = [' The declustering found ' num2str(max(vMain)) ' clusters of earthquakes, a total of '...
        ...
        ' events out of ' num2str(mCatalog.Count) '. '...
...
axes(axs1(1));
[vFreqMag] = hist(mCatAfter(:,6),(0:0.1:max(mCatalog.Magnitude)));
histogram(mCatAfter(:,6),(0:0.1:max(mCatalog.Magnitude)));
set(gca,'Xlim',[0 ceil(max(mCatalog.Magnitude))],'Ylim',[0 ceil(max(vFreqMag))]);
