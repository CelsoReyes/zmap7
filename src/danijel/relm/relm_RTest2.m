function [rRelmTest] = relm_LTest2(vRatesH, vRatesN, nNumberSimulation, fMagThreshold, bOptimized, bDrawFigure)
...
%   vSimNum_N = sum(calc_logpoisspdf(vNum_N, vLambdaN)) - fLikelihood_N;
  ...
  for nCnt = 1:nNumberSimulation
    ...
    vNum_H = poissinv(vRandom, vLambdaH);
    vNum_N = poissinv(vRandom, vLambdaN);
    vNum_H = (sum(calc_logpoisspdf(vNumberQuake, vNum_H)));
    vNum_N = (sum(calc_logpoisspdf(vNumberQuake, vNum_N)));
