...
if newMapWindowFlag
    ...
    omenu = uimenu(symbolmenu,'Label',' Overlay ');
    uimenu(omenu,'Label','Show major rivers','Callback','domask = ''rivers''; overlay3');
    uimenu(omenu,'Label','Show major cities','Callback','domask = ''cities''; overlay3');
    uimenu(omenu,'Label','Show coast and Nationa Boudaries','Callback','domask = ''coast''; overlay3');
    uimenu(omenu,'Label',' Do not show volcanoes ','Callback','vo = [];subcata');
    ...
    TypeMenu = uimenu(symbolmenu,'Label',' Mag by size and depth by color ',...
    ...
    uimenu(options,'Label','Select EQ in Polygon (Menu) ',...
        'Callback','h1 = gca;newt2 = a; stri = ''Polygon''; keyselect');
    ...
    uimenu(op2,'Label','Keep this catalog in memory (use reset to recall)',...
        'Callback','org2 = a; ');
    ...
    uimenu(op2,'Label','Compare two catalogs ',...
    ...
    catSave =...
        ...
        '[file1,path1] = uiputfile([my_dir fs ''eq_data'' fs ''*.mat''], ''Earthquake Datafile'');',...
        'if length(file1) > 1 , sapa2 = [''save '' path1 file1 '' a faults main mainfault coastline infstri well''],',...
        'eval(sapa2) ,end, done'];
    ...
    uimenu(op3B,'Label','Calculate a Mc and b-value Map ',...
    ...
    uimenu(op3B,'Label','Load a b-value depth ratio grid',...
        'Callback','sel= ''lo''; use_old_win = 0; lab1 = ''b ratio'';,bdepth_ratio')
...
if typele == 'mad'
    for i = 1:length(a)
        ...
        col = ceil(a(i,7)*fac)+1; if col > 63; col = 63; end
        set(pl,'Markersize',sm,'markerfacecolor',[c(col,:)]);
    end
end
