...
inpara=[taumin;taumax;xk;xmeff;P;rfact;err;derr];
...
if ~find(clus~=0)
    welcome('Alert','No Cluster found')
    close(wai)
    return
else
    ...
    %save the declustered catalog with all necessary information
    %if exist('ijma')  == 0
    %[file1,path1] = uiputfile([hoda '*.mat'],'Save Cluster Datafile');
    %if length(file1) > 1
    %  sapa2 = ['save ' path1 file1  '  a clus inpara clust newcat',...
    %         ' original bg mbg cluslength bgevent equi ',...
    %         ' eqtime cluscat faults',...
    %       ' coastline main mainfault name ']
    %  eval(sapa2)
    %end
    %end % exist ijma
