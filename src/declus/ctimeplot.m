function ctimeplot() 
    ...
    function create_my_menu()
        ...
        uimenu(options,'Label','Cuts in magnitude and depth','MenuSelectedFcn',@cb_cut_mag_depth)
        uimenu (options,'Label','Decluster the catalog','MenuSelectedFcn',@(~,~)ReasenbergDeclusterClass(catalog));
        uimenu(options,'Label','AS(t)function','MenuSelectedFcn',@(~,~)newsta('ast',ZG.newt2));
        uimenu(options,'Label','Rubberband function','MenuSelectedFcn',@(~,~)newsta('rub',ZG.newt2));
        uimenu(options,'Label','LTA(t) function ','MenuSelectedFcn',@(~,~)newsta('lta',ZG.newt2));
        uimenu(options,'Label','Overlay another curve (hold)','MenuSelectedFcn',@cb_overlayanothercurve)
        uimenu(options,'Label','Compare two rates ( No fit)','MenuSelectedFcn',@(~,~)dispma3())
        
        op4 = uimenu(options,'Label','b-value estimation');
        uimenu(op4,'Label','manual','MenuSelectedFcn',@(~,~)bfitnew(ZG.newt2))
        uimenu(op4,'Label','automatic','MenuSelectedFcn',@(~,~)bdiff(ZG.newt2))
        uimenu(op4,'Label','b with depth','MenuSelectedFcn',@(~,~)bwithde(ZG.newt2))
        uimenu(op4,'Label','b with time','MenuSelectedFcn',@(~,~)bwithti(ZG.newt2))
        
        op5 = uimenu(options,'Label','p-value estimation');
        uimenu(op5,'Label','manual','MenuSelectedFcn',@cb_man_pval_estimation)
        uimenu(op5,'Label','automatic','MenuSelectedFcn',@cb_auto_pval_estimation)
        uimenu(options,'Label','get coordinates with Cursor','MenuSelectedFcn',@cb_getcoord_with_cursor)
        uimenu(options,'Label','Cumlative Moment Release ','MenuSelectedFcn',@cb_cum_moment_release)
        uimenu(options,'Label','Time Selection','MenuSelectedFcn',@cb_time_selection);
        %uimenu(options,'Label',' Magnitude signature','MenuSelectedFcn',@cb_mag_signature)
        uimenu(options,'Label','Save cumulative number curve','MenuSelectedFcn',@cb_save_cumnumcurve)
