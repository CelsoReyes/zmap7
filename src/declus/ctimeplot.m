function ctimeplot() % autogenerated function wrapper
    ...
    function create_my_menu()
        ...
        uimenu(options,'Label','Cuts in magnitude and depth',Futures.MenuSelectedFcn,@cb_cut_mag_depth)
        uimenu (options,'Label','Decluster the catalog',Futures.MenuSelectedFcn,@(~,~)inpudenew(catalog));
        uimenu(options,'Label','AS(t)function',Futures.MenuSelectedFcn,@(~,~)newsta('ast'));
        uimenu(options,'Label','Rubberband function',Futures.MenuSelectedFcn,@(~,~)newsta('rub'));
        uimenu(options,'Label','LTA(t) function ',Futures.MenuSelectedFcn,@(~,~)newsta('lta'));
        uimenu(options,'Label','Overlay another curve (hold)',Futures.MenuSelectedFcn,@cb_overlayanothercurve)
        uimenu(options,'Label','Compare two rates ( No fit)',Futures.MenuSelectedFcn,@(~,~)dispma3())
        
        op4 = uimenu(options,'Label','b-value estimation');
        uimenu(op4,'Label','manual',Futures.MenuSelectedFcn,@(~,~)bfitnew(ZG.newt2))
        uimenu(op4,'Label','automatic',Futures.MenuSelectedFcn,@(~,~)bdiff(ZG.newt2))
        uimenu(op4,'Label','b with depth',Futures.MenuSelectedFcn,@(~,~)bwithde(ZG.newt2))
        uimenu(op4,'Label','b with time',Futures.MenuSelectedFcn,@(~,~)bwithti(ZG.newt2))
        
        op5 = uimenu(options,'Label','p-value estimation');
        uimenu(op5,'Label','manual',Futures.MenuSelectedFcn,@cb_man_pval_estimation)
        uimenu(op5,'Label','automatic',Futures.MenuSelectedFcn,@cb_auto_pval_estimation)
        uimenu(options,'Label','get coordinates with Cursor',Futures.MenuSelectedFcn,@cb_getcoord_with_cursor)
        uimenu(options,'Label','Cumlative Moment Release ',Futures.MenuSelectedFcn,@cb_cum_moment_release)
        uimenu(options,'Label','Time Selection',Futures.MenuSelectedFcn,@cb_time_selection);
        %uimenu(options,'Label',' Magnitude signature',Futures.MenuSelectedFcn,@cb_mag_signature)
        uimenu(options,'Label','Save cumulative number curve',Futures.MenuSelectedFcn,@cb_save_cumnumcurve)
        calSave =...
