function meant() % autogenerated function wrapper
    %  meant  finds the average depth for a predefined running
    ...
    if ic == 1 | ic == 0
        ...
        close_button=uicontrol('Style','Pushbutton',...
            ...
            'Units','normalized','callback',@cb_cancel,'String','Cancel');
    ...
    elseif ic == 2
        ...
        if isempty(ZG.newcat) ZG.newcat = a; end
        %winlen_days = round(iwln* 365/ZG.bin_dur);    % window length in years is converted to bins
        len = ZG.newcat.Count;
        xt2  = [ ];
        meand = [ ];
        er = [];
        ...
        if isempty(depfg)
            ...
            uicontrol('Style','Pushbutton',...
                'Position',[.9 .80 .10 .05],...
                'Units','normalized',...
                'callback',{@cb_medispas1,'ast'},'String','AS');
            uicontrol('Style','Pushbutton',...
                'Position',[.9 .70 .10 .05],...
                'Units','normalized',...
                'callback',{@cb_medispas1,'lta'},'String','LTA');
            uicontrol('Style','Pushbutton',...
                'Position',[.9 .90 .10 .05],...
                'Units','normalized',...
                'callback',@cb_COMbutton,'String','Com');
            
            new = uicontrol('style','edit','value',winlen_days,...
                'string',num2str(winlen_days), 'background','y',...
                'callback',@cb_winlen_days,...
    ...
    function cb_medispas1(mysrc,myevt,mysta)
        sta=mysta;
        medispas1;
    end
    
    function cb_COMbutton(mysrc,myevt)
        callback_tracker(mysrc,myevt,mfilename('fullpath'));
        dispma4;
    end
    
    function cb_winlen_days(mysrc,myevt)
        callback_tracker(mysrc,myevt,mfilename('fullpath'));
        winlen_days=str2num(mysrc.String);
        medispas1;
    end
