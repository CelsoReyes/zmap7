function varargout = sr_analyze(varargin)
...
% Last Modified by GUIDE v2.5 31-Aug-2005 14:39:48
...
axes(handles.axsCumVsMag);
set(handles.axsCumVsMag, 'NextPlot', 'replace');
set(handles.axsCumVsTime, 'NextPlot', 'replace');
if get(handles.radEntireTime, 'Value') == 1
    axes(handles.axsCumVsMag)
    [n_,x_]=histogram(mPlotCatalog(:,6),0:0.2:ceil(max(mPlotCatalog(:,6))));
    semilogy(x_,n_,'k^','era','xor');
  %  [vDummy,vIndices] = sort(mPlotCatalog
  % Plot the frequency magnitude distribution
%   [hPlot, fBValue, fAValue, fStdDev, fMc] = plot_FMD(mPlotCatalog, 1, handles.axsCumVsMag, 's', 'k', 1, handles.params.rOptions.nCalculateMC, handles.params.rOptions.fBinning);
...
%   set(handles.lstValues, 'String', sList);
  ...
  [hPlot, fBValue1, fAValue, fStdDev, fMc] = plot_FMD(mFirst, 1, handles.axsCumVsMag, 's', 'k', 1, handles.params.rOptions.nCalculateMC, handles.params.rOptions.fBinning);
  ...
  set(handles.axsCumVsMag, 'NextPlot', 'add');
  [hPlot, fBValue2, fAValue, fStdDev, fMc] = plot_FMD(mSecond, 1, handles.axsCumVsMag, '^', 'r', 1, handles.params.rOptions.nCalculateMC, handles.params.rOptions.fBinning);
  set(hPlot, 'MarkerSize', 3);
  ...
  axes(handles.axsCumVsTime)
  set(handles.axsCumVsTime, 'NextPlot', 'add');
...
[fX, fY] = ginput(1);
[fX fY,  nNodeGridPoint] = calc_ClosestGridNode(handles.params.mPolygon, fX, fY);
...
if handles.params.nGriddingMode == 0
