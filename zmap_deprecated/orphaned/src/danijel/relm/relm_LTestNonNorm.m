...
if bOptimized
  ...
  vSimNum_H = sum(calc_logpoisspdf(vNum_H, vLambdaH));
  vSimNum_N = sum(calc_logpoisspdf(vNum_N, vLambdaN));
else
  ...
  for nCnt = 1:nNumberSimulation
    ...
    vNum_H = poissinv(vRandom, vLambdaH);
    vNum_N = poissinv(vRandom, vLambdaN);
    vSimNum_H = [vSimNum_H; (sum(calc_logpoisspdf(vNum_H, vLambdaH)))];
    vSimNum_N = [vSimNum_N; (sum(calc_logpoisspdf(vNum_N, vLambdaN)))];
  end
end
