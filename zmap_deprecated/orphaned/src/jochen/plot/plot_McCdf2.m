function [mResult, fMls, fMc, fMu, fSigma, mDatPredBest, vPredBest] = plot_McCdf2(mCatalog, fBinning)
% function  [mResult, fMls, fMc, fMu, fSigma, mDatPredBest, vPredBest] = plot_McCdf2(mCatalog, fBinning);
% -----------------------------------------------------------------------------------------------------
% Same as calc_McCdf2 with plotting the fitting steps and the final result
%
% Incoming variables:
% mCatalog   : EQ catalog
...
% Calculate FMD for original catalog
[vFMDorg, vNonCFMDorg] = calc_FMD(mCatalog);
for fMc = fMcBound-0.8:0.1:fMcBound+0.8
    fMc = round(fMc*10)/10;
    vFMD = vFMDorg;
    ...
    if (length(mCatalog(vSel,1)) >= 20)
        ...
        if (~isempty(fNmax) & ~isnan(fNmax) & fNmax ~= 0 & length(mDataTest(:,1)) > 4)
            ...
            [vX, resnorm, resid, exitflag, output, lambda, jacobian]=lsqcurvefit(@calc_lognormal,[fSmu  fSSigma], mDataTest(:,2), mDataTest(:,3));
...
sTitlestr = ['Mc = ' num2str(fMc) ' using Lognormal CDF fitting'];
title(sTitlestr)
xlabel('Magnitude');
ylabel('Non-cumulative FMD');
drawnow;
