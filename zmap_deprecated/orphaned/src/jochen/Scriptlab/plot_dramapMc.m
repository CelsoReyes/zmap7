function []=plot_dramapMc(vResults)
% --------------------------------------------
...
% Harvard
%  vSel = find(vResults.mValueGrid(:,7)> min(vResults.mValueGrid(:,7)));%(5.2);
%  vResults.mValueGrid(~vSel,7) = NaN;
vSel = find(vResults.mValueGrid(:,7)> 6);
vResults.mValueGrid(vSel,7) = NaN;
% Reshape
mMcValues=reshape(vResults.mValueGrid(:,7),length(vY),length(vX));
% Interpolate Values
...
caxis([5.2 6]) % Harvard 0-70km
%caxis([ min(min(mMcValues)) 5.5]) % ISC 0-70km
hColormap = colormap(j);
set(hColormap,'Position',[0.93 0.3 0.01 0.4]);
brighten(0.1);
