function [result]=sv_NodeCalc2(mCatalog, fSplitTime, bTimePeriod, fTimePeriod, nCalculateMC, vCluster, bDecluster, fMcOrg, fBValueOrg, bSeisModel)
% function [result]=sv_NodeCalc2(mCatalog, fSplitTime, bTimeperiod, fTimePeriod, nCalculateMC, vCluster, bDecluster, fMcOrg, fBValueOrg, bSeisModel)
% -------------------------------------------------------------------------------------------------------------------------------------------------
...
elseif (bSeisModel == 1)
    ...
    %% Determination of pecentage change in seismicity between two time periods normalized to year
    ...
    result.dNdiff = max(dEv_valsum2)/result.fSecondPeriodExact/(max(dEv_valsum)/result.fFirstPeriodExact); % Normalization
    ...
    [dEv_val dMags dEv_valsum dEv_valsum_rev,  dMags_rev] = calc_cumulsum(result.mFirstCatalog);
    [dEv_val2 dMags2 dEv_valsum2 dEv_valsum_rev2,  dMags_rev2] = calc_cumulsum(result.mSecondCatalog);
    result.dNdiff = abs(dEv_valsum2./result.fSecondPeriodExact-dEv_valsum./result.fFirstPeriodExact);
    ...
    % Determine Mc for entire catalog, two time periods and difference in Mc
    %[fMc, fStd_Mc, fBvalue, fStd_B, fAvalue, fStd_A, vMc, mBvalue] = calc_McBboot(mCatalog, fBinning, nSample, nCalculateMc)
    result.fMc = calc_Mc(mCatalog, nCalculateMC);
    if isempty(result.fMc)
        result.fMc = NaN;
    end
    ...
    % NO MODELS WITH STRETCH ALLOWED so far
    %%% Maximum likelihood scores and corrected AICs
    [fProb_nochange, fAICc_nochange] = calc_loglikelihood_nochange2(result.mFirstCatalog, result.mSecondCatalog);
    [fdM, fProb_dM, fAICc_dM, mLikeli_dM] = calc_loglikelihood_dM2(result.mFirstCatalog, result.mSecondCatalog);
    [fdMc, fProb_dMc, fAICc_dMc] = calc_llhd_dMc2(result.mFirstCatalog, result.mSecondCatalog);
    %[fS, fProb_stretch, fAICc_stretch, mLikeli_dS] = calc_loglikelihood_stretch(result.mFirstCatalog, result.mSecondCatalog);
    [fFac, fProb_Rate, fAICc_Rate, mLikeli_Rate] = calc_loglikelihood_rate2(result.mFirstCatalog, result.mSecondCatalog);
    [fdM_rate, fdM_Fac, fProb_dMrate, fAICc_dMrate, mLikeli_dMrate] = calc_loglikelihood_dM_rate2(result.mFirstCatalog, result.mSecondCatalog);
    %[fdS_rate, fdS_Fac, fProb_dSrate, fAICc_dSrate mLikeli_dSrate] = calc_loglikelihood_stretch_rate(result.mFirstCatalog, result.mSecondCatalog);
    %[fdM_st, fStretch, fProb_Trans, fAICc_Trans, mLikeli_Trans] = calc_loglikelihood_Trans(result.mFirstCatalog, result.mSecondCatalog);
    %[fdM_all, fdS_all, fFac_all, fProb_all, fAICc_all, mLikeli_all] = calc_loglikelihood_dMdSrate(result.mFirstCatalog, result.mSecondCatalog);
