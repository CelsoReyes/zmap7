function [mResult, fMls, fMc, fMu, fSigma, mDatPredBest, vPredBest] = calc_McCdf2(mCatalog, fBinning)
% function  [mResult, fMls, fMc, fMu, fSigma, mDatPredBest, vPredBest] = calc_McCdf2(mCatalog, fBinning);
% -----------------------------------------------------------------------------------------------------
...
for fMc = fMcBound-0.8:0.1:fMcBound+0.8
    ...
    if (length(mCatalog(vSel,1)) >= 20)
        ...
        vFMD(2,:) = vFMD(2,:)./fPeriod1; % ceil commented out
        vNonCFMD(2,:) = vNonCFMD(2,:)./fPeriod1; % ceil commented out
