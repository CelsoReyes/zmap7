function [mCatDecluster, mCatAfter, vCluster, vCl, vMainCluster] = calc_decluster_ver2(mCatalog,nMethod)
% function [mCatDecluster, mCatAfter, vCluster, vCl, vMainCluster] = calc_decluster_ver2(mCatalog,nMethod)
% ----------------------------------------------------------------------------------------------------------
%
...
% 28.08.02: Inserted waitbars
% 28.08.02: Changed distance determination using now distance and repmat

%%% Remember: Time difference mainshock-original event not implemented. This is done in version calc_decluster.m
%%%           Some clusters too long...
    ...
    if vCluster(nEvent) == 0
        ...
        if fMagnitude(nEvent) >= fMagThreshold
            ...
            if isnan(mCatalog(nEvent,1))
                ...
                mPos = repmat(mPos, length(mCatalog(:,1)), 2);
            ...
            if length(vIndiceSingle) == 1
                %nCount
                %vIndiceSingle
                vCluster(vIndiceSingle)=0; % Set the event as mainshock
