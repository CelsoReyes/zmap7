...
if oldfig_button == 0
    ...
    uimenu(options,'Label','Select EQ in Circle',...
         'Callback','h1 = gca;met = ''ni''; ho=false;cirbva;watchoff(bmap)')
    uimenu(options,'Label','Select EQ in Circle - Constant R',...
         'Callback','h1 = gca;met = ''ra''; ho=false;cirbva;watchoff(bmap)')
...
uimenu(recmenu,'Label','recurrence time map ',...
     'Callback','def = {''6''};m = inputdlg(''Magnitude of projected mainshock?'',''Input'',1,def);m1 = m{:}; m = str2num(m1);lab1 = ''Tr (yrs) (sm. values only)'';re3 =(teb - t0b)./(10.^(avm-m*old)); mrt = m; view_bva') % was defined as makert

uimenu(recmenu,'Label','(1/Tr)/area map ',...
     'Callback','def = {''6''};m = inputdlg(''Magnitude of projected mainshock?'',''Input'',1,def);m1 = m{:}; m = str2num(m1);lab1 = ''1/Tr/area '';re3 =(teb - t0b)./(10.^(avm-m*old)); re3 = 1./re3/(2*pi*ra*ra);  mrt = m; view_bva') % was defined as makertp
...
uimenu(op1,'Label','Resolution map',...
     'Callback','lab1=''Radius in [km]'';re3 = reso; view_bva')
